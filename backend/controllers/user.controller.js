...
export const getUserProfile = async (req, res) => {
  try {
    ...
    res
      .status(200)
      .json({ success: true, message: 'User found successfully', user: { ...user._doc, password: undefined } });
  } catch (error) {
...
export const getSuggestedUsers = async (req, res) => {
  try {
    ...
    res.status(200).json({
      ...
      users: suggestedUsers.map((user) => ({ ...user, password: undefined })),
    });
  } catch (error) {
...
export const getSearchedUsers = async (req, res) => {
  try {
    ...
    res.status(200).json({ success: true, message: 'Searched successfully', users });
  } catch (error) {
...
export const followUnfollowUser = async (req, res) => {
  try {
    ...
    if (!userToModify || !currentUser) return res.status(400).json({ success: false, message: 'User not found' });
    ...
    if (isFollowing) {
      ...
      const notification = new Notification({ from: userId, to: userToModify._id, type: 'follow' });
